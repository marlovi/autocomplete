...

{
    ...
    function relatorioIncompletas()
    {
        ...
        // OK buscar todos registros de entrada
        // OK fazer uma lista com todos id de entrada
        ...
        $sqlEntrada = "SELECT  p.`status` , p.`id_pesagem`,p.`veiculo_id_veiculo`, v.`placa`  FROM `pesagem` as `p`, `cliente` as `c`, `fornecedor` as `f`, `veiculo` as `v` WHERE p.`status` = 0  AND p.`cliente_id_cliente` = c.`id_cliente` AND f.`id_fornecedor` = p.`fornecedor_id_fornecedor`  AND v.`id_veiculo` = p.`veiculo_id_veiculo` ORDER BY `p`.`veiculo_id_veiculo` ASC";
    ...
    function consultaPesagemOperacao($operacao)
    {
        ...
        if (isset($operacao->data_1) === true) {
            $data1 = str_replace("/", "-", $operacao->data_1); // aqui tiro as / e coloco -
            $_firstDate = date("d-m-Y", strtotime($data1)); // aqui crio a variavel tipo data
            $dataInicial = date("Y-m-d", strtotime($_firstDate)); // aqui mudo a posição mes com dia brasil para usa

            // tratar data2

            $data2 = str_replace("/", "-", $operacao->data_2); // aqui tiro as / e coloco -
            $_secondDate = date("d-m-Y", strtotime($data2)); // aqui crio a variavel tipo data
            $dataFinal = date("Y-m-d", strtotime($_secondDate)); // aqui mudo a posição mes com dia brasil para usa
            $tipo_operacao = 500; // consulta por PERIODO
...
WHERE p.`cliente_id_cliente` = c.`id_cliente` AND f.`id_fornecedor` = p.`fornecedor_id_fornecedor` AND v.`id_veiculo` = p.`veiculo_id_veiculo` AND pro.`id_produto` = p.`produto_id_produto` AND (p.`data` between '$dataInicial'   AND '$dataFinal') ORDER BY p.`produto_id_produto`  ASC  ";
            $result = $conn->query($sql);
        }
        ...
        if (isset($operacao->saida) && isset($operacao->entrada)) {
            ...
            if ($operacao->entrada === true && $operacao->saida === true) {
                ...
                if ($solicitadoPagina === 3) {
                    $tipo_operacao = 0;
                    $sqlEntrada = " SELECT DATE_FORMAT(p.`data`, '%d-%m-%Y ') AS data , p.`status` , p.`cliente_id_cliente` , p.`fornecedor_id_fornecedor` , p.`produto_id_produto` , p.`motorista` , p.`observacao`, p.`id_pesagem`, p.`peso_1`, p.`peso_2`, p.`peso_descontos`, p.`peso_liquido`, c.`nome`AS `cliente` , c.`cpf`AS `cpf_cliente` , c.`cnpj`AS `cnpj_cliente`, f.`nome` AS `fornecedor` , f.`cpf` AS `cpf_fornecedor` , f.`cnpj` AS `cnpj_fornecedor`, v.`placa`, pro.`nome` AS `produto` , pro.`id_produto` AS `cod_prod`  FROM `pesagem` as `p`, `cliente` as `c`, `fornecedor` as `f`, `veiculo` as `v`, `produto` as `pro` WHERE p.`status` = " . $tipo_operacao . "  AND p.`cliente_id_cliente` = c.`id_cliente` AND f.`id_fornecedor` = p.`fornecedor_id_fornecedor`  AND v.`id_veiculo` = p.`veiculo_id_veiculo` AND pro.`id_produto` = p.`produto_id_produto` ";
                    $resultEntrada = $conn->query($sqlEntrada);
                    $tipo_operacao = 1;
                    $sqlSaida = " SELECT DATE_FORMAT(p.`data`, '%d-%m-%Y ') AS data , p.`status` , p.`cliente_id_cliente` , p.`fornecedor_id_fornecedor` , p.`produto_id_produto` , p.`motorista` , p.`observacao`, p.`id_pesagem`, p.`peso_1`, p.`peso_2`, p.`peso_descontos`, p.`peso_liquido`, c.`nome`AS `cliente` , c.`cpf`AS `cpf_cliente` , c.`cnpj`AS `cnpj_cliente`, f.`nome` AS `fornecedor` , f.`cpf` AS `cpf_fornecedor` , f.`cnpj` AS `cnpj_fornecedor`, v.`placa`, pro.`nome` AS `produto` , pro.`id_produto` AS `cod_prod`  FROM `pesagem` as `p`, `cliente` as `c`, `fornecedor` as `f`, `veiculo` as `v`, `produto` as `pro` WHERE p.`status` = " . $tipo_operacao . "  AND p.`cliente_id_cliente` = c.`id_cliente` AND f.`id_fornecedor` = p.`fornecedor_id_fornecedor`  AND v.`id_veiculo` = p.`veiculo_id_veiculo` AND pro.`id_produto` = p.`produto_id_produto` ";
                ...
                if ($solicitadoPagina === 316) {
                    ...
                    $sqlEntrada = " SELECT DATE_FORMAT(p.`data`, '%d-%m-%Y ') AS data , p.`status` , p.`cliente_id_cliente` , p.`fornecedor_id_fornecedor` , p.`produto_id_produto` , p.`motorista` , p.`observacao`, p.`id_pesagem`, p.`peso_1`, p.`peso_2`, p.`peso_descontos`, p.`peso_liquido`, c.`nome`AS `cliente` , c.`cpf`AS `cpf_cliente` , c.`cnpj`AS `cnpj_cliente`, f.`nome` AS `fornecedor` , f.`cpf` AS `cpf_fornecedor` , f.`cnpj` AS `cnpj_fornecedor`, v.`placa`, pro.`nome` AS `produto` , pro.`id_produto` AS `cod_prod`  FROM `pesagem` as `p`, `cliente` as `c`, `fornecedor` as `f`, `veiculo` as `v`, `produto` as `pro`  WHERE p.`status` = " . $tipo_operacao . " AND v.`placa` LIKE '" . $placa . "%'  AND  p.`cliente_id_cliente` = c.`id_cliente` AND f.`id_fornecedor` = p.`fornecedor_id_fornecedor`  AND v.`id_veiculo` = p.`veiculo_id_veiculo` AND pro.`id_produto` = p.`produto_id_produto`  ";
                    $resultEntrada = $conn->query($sqlEntrada);
                    $tipo_operacao = 1; //DATE_FORMAT(p.`data`, '%d-%m-%Y %h:%i:%s')
                    $sqlSaida = " SELECT DATE_FORMAT(p.`data`, '%d-%m-%Y ') AS data , p.`status` , p.`cliente_id_cliente` , p.`fornecedor_id_fornecedor` , p.`produto_id_produto` , p.`motorista` , p.`observacao`, p.`id_pesagem`, p.`peso_1`, p.`peso_2`, p.`peso_descontos`, p.`peso_liquido`, c.`nome`AS `cliente` , c.`cpf`AS `cpf_cliente` , c.`cnpj`AS `cnpj_cliente`, f.`nome` AS `fornecedor` , f.`cpf` AS `cpf_fornecedor` , f.`cnpj` AS `cnpj_fornecedor`, v.`placa`, pro.`nome` AS `produto` , pro.`id_produto` AS `cod_prod`  FROM `pesagem` as `p`, `cliente` as `c`, `fornecedor` as `f`, `veiculo` as `v`, `produto` as `pro`  WHERE p.`status` = " . $tipo_operacao . " AND v.`placa` LIKE '" . $placa . "%'  AND  p.`cliente_id_cliente` = c.`id_cliente` AND f.`id_fornecedor` = p.`fornecedor_id_fornecedor`  AND v.`id_veiculo` = p.`veiculo_id_veiculo` AND pro.`id_produto` = p.`produto_id_produto`  ";
                    $resultSaida = $conn->query($sqlSaida);
                }
            }
    ...
    function getPesagem($id_pesagem)
    {
    ...
    function consultaPesagemPlaca($id_placa)
    {
    ...
    function consultaPesagemCliente($id_placa)
    {
    ...
    function consultaPesagemFornecedor($id_placa)
    {
        $banco = new Banco();
        $teste = $banco->serverName;
        $conn = new mysqli($banco->serverName, $banco->user, $banco->password, $banco->dataBase);
