    ...
    $scope.printDiv_relatorio = function() {
        console.log("relatoriomovimentacaoController :printDiv_relatorio");
    ...
    $scope.gerarTituloTabela = function() {
     ...
     $scope.gerarCorpoTabelamovimentacao = function(consulta) {
        ...
        console.log("relatoriomovimentacaoController: gerarCorpoTabelamovimentacao ");
    ...
    } //fim gerarCorpoTabelamovimentacao
    // VARIAVEIS GLOBAIS
...
/*
   $scope.relatoriomovimentacao = function() {
        console.log("OBJETO: MOVIMENTACAO",$scope.movimentacao);
...
*/
    ...
    $scope.relatoriomovimentacao = function() {
        console.log("OBJETO: MOVIMENTACAO",$scope.movimentacao);
        console.log("relatoriomovimentacaoController :relatoriomovimentacao");
        console.log("antes de iteragir teste: ",$scope.teste);
        $('#divdinamica').empty();
        var request = $http({
            method: "post",
            url: "php/pesagem/pesquisarpesagem.php",
            ...
            if ($scope.consulta.resultado == 0) {
                ...
                angular.forEach($scope.nome_produto_anterior, function(value, key1) {
                    ...
                    angular.forEach(response.data, function(value, key) {
                        ...
                        if (response.data[key].status === "SAIDA" && response.data[key].produto_id_produto === $scope.nome_produto_anterior[key1]) {
                            $scope.resultado_peso_2 = $scope.resultado_peso_2 + parseInt(response.data[key].peso_2);
                            $scope.resultado_liquido = $scope.resultado_liquido + parseInt(response.data[key].peso_liquido);
                            $scope.corpo_tabela_entrada = (value);
                        }
                    });
...
console.log("contador de clicks: ",$scope.movimentacao.click);

                 if ($scope.movimentacao.click === 2) {
                    $scope.movimentacao.click = 0;
                    $scope.consulta = [];
                    $scope.id_produto_anterior = [];
                    $scope.nome_produto_anterior = [];
                    $scope.corpo_tabela_entrada = [];
                    $scope.consulta.resultado = [];
                    $scope.resultado_peso_1 = 0;
                    $scope.resultado_peso_2 = 0;
                    $scope.resultado_liquido = 0;
                    $scope.myText = [];
                    var $newDiv = [];
                 ...
                 } // fim if que identifica segundo click botao
