...
//for (var i in listaveiculos) {
// consultaPesagem(listaveiculos[i]); 
//}
                                                      ...
                                                      headers: {
                                                        ...
                                                        request.then(function(response) {
                                                          //  console.log("resposta da consulta da ultima pesagem de entrada");
                                                            ...
                                                            // AQUI ENTRA AS PESAGENS INCOMPLETAS
                                                                       // $('#divdinamica').empty();
                                                                       // $scope.myText = $scope.gerarTituloTabela();// gera titulo tabela uma
                                                                       // $scope.myText = $scope.myText + $scope.gerarCorpoTabelaIncompletas(response.data);
                                                                      // console.log($scope.incompletas);
                                                        }, function(response) {
                                                                console.log("ERROR" + response);
                                                            });
...
/*
    ...
    $scope.relatorioincompletas2 = function() {

if( $scope.incompletas.cont <= 1){
                 consultaPesagemIncompletasManual($scope.incompletas.cont); // AQUI TESTA O ID DO VEICULO
               $('#divdinamica').empty();
               $scope.myText = $scope.gerarTituloTabela();// gera titulo tabela uma
                 for(var i in $scope.incompletas){
                           if($scope.incompletas[i].hasOwnProperty('id_pesagem')){
                 ...
                 $scope.incompletas.cont = $scope.incompletas.cont +1;

              }


  function consultaPesagemIncompletasManual(item) {
console.log("relatorioincompletasController :consultaPesagemIncompletasManual");
// FUNÇÃO QUE FAZ LISTA DE VEICULOS CADASTRADOS
var request = $http({
        method: "post",
        url: "php/veiculo/pesquisarveiculo.php",
    ...
    request.then(function(response) {
       ...
       // console.log(response.data);
      for (var i in response.data) {
       // console.log(response.data[i].id_veiculo);
        consultaPesagem(response.data[i].id_veiculo);
      }
    }, function(response) {
        console.log("ERROR" + response);
    });
// consultaPesagem(item);
   function consultaPesagem(item) {
      // item = 129; // VEICULO COM PESAGEM DE ENTRADA EM ABERTO
    //item = 131; // VEICULO COM PESAGEM DE ENTRADA OK
    // FUNÇÃO QUE CONSULTA AS PESAGENS RELACIONADAS POR ID VEICULO PLACA
      ...
      //  console.log(cont_pesagem_saida);
        ...
        if (cont_pesagem_entrada === cont_pesagem_saida) {
         //   console.log("NÃO EXISTE Pesagem em aberto");
           // Materialize.toast(' SEM OPERAÇÕES INCOMPLETAS!', 3000, 'rounded', 'center');
            //Materialize.toast();
        } else {
                  if (cont_pesagem_entrada >= cont_pesagem_saida) {
                        //  console.log("permitido pesagem de SAIDA");
                  ...
                  request.then(function(response) {
                    //  console.log("resposta da consulta da ultima pesagem de entrada");
                      Array.prototype.push.apply($scope.incompletas, response.data);
                      //this.incompletas = $scope.incompletas;
                                 // $('#divdinamica').empty();
                                 // $scope.myText = $scope.gerarTituloTabela();// gera titulo tabela uma
                                 // $scope.myText = $scope.myText + $scope.gerarCorpoTabelaIncompletas(response.data);
                                // console.log($scope.incompletas);
   ...
   } // fim de consultaPesagem(item) 
   
  }// FIM consultaPesagemIncompletasManual
 // $scope.incompletas = [] ;
    } // FIM FUNÇÃO RELATORIO INCOMPLETAS
 */
}
   angular
       .module('home')
